...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const { count } = await supabase
    ...
    .eq('vehicle_type', 'car')
  return {
    title: `Coches ${data.name} en venta en España`,
    description: `Coches ${data.name} de segunda mano en España. Concesionarios y especialistas verificados, con historial documentado y ficha técnica completa.`,
    alternates: { canonical: `/marcas/${brand}/coches` },
...
export default async function BrandCochesPage({ params }: PageProps) {
  ...
  const itemListJsonLd = vehicles?.length
    ? {
        ...
        '@type': 'ItemList',
        name: `Coches ${brandData.name} en venta en España`,
        url: `${SITE_URL}/marcas/${brand}/coches`,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 4, name: 'Coches' },
    ],
  }
    ...
    <div className="max-w-screen-2xl mx-auto px-6 lg:px-12 pt-28 pb-20">
      ...
      <div className="mb-10">
        <nav aria-label="breadcrumb" className="mb-6">
          <ol className="flex items-center gap-1.5 text-xs text-bsm-text-muted">
            ...
            <li className="text-[#3A3A3A]" aria-hidden="true">/</li>
            <li className="text-bsm-text-secondary" aria-current="page">Coches</li>
          </ol>
        </nav>
        ...
        <div className="flex flex-col md:flex-row md:items-end gap-6 mb-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-xs text-gold tracking-widest uppercase">Marketplace · Coches</span>
            </div>
            <h1 className="section-title">Coches {brandData.name} en venta</h1>
          </div>
        </div>
        ...
        <div className="flex gap-3 text-sm">
          ...
          <span className="text-gold">Coches</span>
          <span className="text-[#3A3A3A]">·</span>
      ...
      {vehicles && vehicles.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center border border-bsm-border bg-surface">
          <h2 className="font-display text-xl mb-2 text-bsm-text-primary">Sin coches {brandData.name} disponibles</h2>
          <p className="text-sm text-bsm-text-muted max-w-xs mb-6">
            No hay coches de esta marca en este momento. Explora todos los {brandData.name} o busca tu vehículo a la carta.
          </p>
