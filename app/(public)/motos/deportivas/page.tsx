...
const CATEGORY_VALUES = ['deportivas']
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: 'Motos deportivas en venta en España',
    description: 'Ducati Panigale V4, BMW M1000RR, Kawasaki Ninja H2, Aprilia RSV4, Honda CBR1000RR... Motos deportivas de alto rendimiento de especialistas verificados en España.',
    alternates: { canonical: '/motos/deportivas' },
    ...(count === 0 ? { robots: { index: false } } : {}),
  }
}

export default async function MotosDeportivasPage() {
  const supabase = createPublicClient()
  ...
  const faqItems = buildCategoryFaqItems(MOTO_CATEGORY_FAQ.deportivas, stats, esGroupThousands)

  const itemListJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: 'Motos deportivas en venta en España',
    url: `${SITE_URL}/motos/deportivas`,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Deportivas' },
    ],
  }
  ...
  return (
      ...
      <div className="mb-10">
        <nav aria-label="breadcrumb" className="mb-6">
          <ol className="flex items-center gap-1.5 text-xs text-bsm-text-muted">
            ...
            <li className="text-[#3A3A3A]" aria-hidden="true">/</li>
            <li className="text-bsm-text-secondary" aria-current="page">Deportivas</li>
          </ol>
        </nav>
        ...
        <h1 className="section-title mb-3">Motos deportivas</h1>
        <p className="text-sm text-bsm-text-muted max-w-2xl">
          Ducati Panigale V4, BMW M1000RR, Kawasaki Ninja H2, Aprilia RSV4... Motos deportivas de alto rendimiento, con la documentación y el historial que esperan los compradores más exigentes.
        </p>
      </div>
      ...
      {vehicles && vehicles.length > 0 ? (
        <>
          ...
          <div className="pt-8 border-t border-bsm-border">
            <Link href="/motos?categoria=deportivas" className="text-sm text-gold hover:text-gold-light transition-colors">
              Ver todas las motos deportivas con filtros →
            </Link>
          </div>
        </>
      ) : (
        <div className="flex flex-col items-center justify-center py-20 text-center border border-bsm-border bg-surface">
          ...
          <p className="text-sm text-bsm-text-muted max-w-xs mb-6">
            Aún no hay motos deportivas publicadas. Explora el catálogo general o solicita una búsqueda a la carta.
          </p>
    ...
    <FaqSection items={faqItems} heading="Preguntas frecuentes sobre motos deportivas" eyebrow="Deportivas" />
    <RelatedCategories categories={MOTO_CATEGORY_RELATIONS.deportivas} brands={brandStock} />
    </>
  )
}
