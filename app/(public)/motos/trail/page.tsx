...
const CATEGORY_VALUES = ['trail_premium']
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const { count } = await supabase
    ...
    .eq('category', 'trail_premium')
  return {
    title: 'Motos trail premium en venta en España',
    description: 'Honda Africa Twin, BMW GS, Ducati DesertX, KTM Adventure, Husqvarna Norden... Motos trail premium para asfalto y fuera de carretera de especialistas verificados en España.',
    alternates: { canonical: '/motos/trail' },
...
export default async function MotosTrailPage() {
  ...
  const faqItems = buildCategoryFaqItems(MOTO_CATEGORY_FAQ.trail, stats, esGroupThousands)

  const itemListJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: 'Motos trail premium en venta en España',
    url: `${SITE_URL}/motos/trail`,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Trail premium' },
    ],
  }
  ...
  return (
      ...
      <div className="mb-10">
        <nav aria-label="breadcrumb" className="mb-6">
          <ol className="flex items-center gap-1.5 text-xs text-bsm-text-muted">
            ...
            <li className="text-[#3A3A3A]" aria-hidden="true">/</li>
            <li className="text-bsm-text-secondary" aria-current="page">Trail premium</li>
          </ol>
        </nav>
        ...
        <h1 className="section-title mb-3">Motos trail premium</h1>
        <p className="text-sm text-bsm-text-muted max-w-2xl">
          Honda Africa Twin, BMW F850 GS, Ducati DesertX, KTM 890 Adventure, Husqvarna Norden 901... Motos trail premium preparadas para cualquier terreno, con el nivel de acabado y la fiabilidad que merece el mercado de segunda mano premium.
        </p>
      </div>
      ...
      {vehicles && vehicles.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center border border-bsm-border bg-surface">
          ...
          <p className="text-sm text-bsm-text-muted max-w-xs mb-6">
            Aún no hay motos trail publicadas. Explora el catálogo general o solicita una búsqueda a la carta.
          </p>
    ...
    <FaqSection items={faqItems} heading="Preguntas frecuentes sobre motos trail y maxitrail" eyebrow="Trail y maxitrail" />
    <RelatedCategories categories={MOTO_CATEGORY_RELATIONS.trail} brands={brandStock} />
    </>
  )
}
