...
const CATEGORY_VALUES = ['custom_cruiser']
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const { count } = await supabase
    ...
    .eq('category', 'custom_cruiser')
  return {
    title: 'Motos custom y cruiser en venta en España',
    description: 'Harley-Davidson Heritage, Indian Chief, Triumph Bobber, BMW R18... Motos custom y cruiser de carácter único, de especialistas verificados con historial documentado en España.',
    alternates: { canonical: '/motos/custom' },
    ...(count === 0 ? { robots: { index: false } } : {}),
  }
}

export default async function MotosCustomPage() {
  const supabase = createPublicClient()
  ...
  const faqItems = buildCategoryFaqItems(MOTO_CATEGORY_FAQ.custom, stats, esGroupThousands)

  const itemListJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: 'Motos custom y cruiser en venta en España',
    url: `${SITE_URL}/motos/custom`,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Custom y cruiser' },
    ],
  }
  ...
  return (
      ...
      <div className="mb-10">
        <nav aria-label="breadcrumb" className="mb-6">
          <ol className="flex items-center gap-1.5 text-xs text-bsm-text-muted">
            ...
            <li className="text-[#3A3A3A]" aria-hidden="true">/</li>
            <li className="text-bsm-text-secondary" aria-current="page">Custom y cruiser</li>
          </ol>
        </nav>
        ...
        <h1 className="section-title mb-3">Motos custom y cruiser</h1>
        <p className="text-sm text-bsm-text-muted max-w-2xl">
          Harley-Davidson Heritage, Indian Chief, Triumph Bobber, BMW R18... Motos de personalidad propia, diseñadas para el placer de rodar y para destacar. Todos los vehículos con historial documentado y vendedores verificados.
        </p>
      </div>
      ...
      {vehicles && vehicles.length > 0 ? (
        <>
          ...
          <div className="pt-8 border-t border-bsm-border flex flex-wrap items-center justify-between gap-4">
            <Link href="/motos?categoria=custom_cruiser" className="text-sm text-gold hover:text-gold-light transition-colors">
              Ver todas las motos custom con filtros →
            </Link>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center border border-bsm-border bg-surface">
          ...
          <p className="text-sm text-bsm-text-muted max-w-xs mb-6">
            Aún no hay motos custom publicadas. Explora el catálogo general o solicita una búsqueda a la carta.
          </p>
    ...
    <FaqSection items={faqItems} heading="Preguntas frecuentes sobre motos custom y cruiser" eyebrow="Custom y cruiser" />
    <RelatedCategories categories={MOTO_CATEGORY_RELATIONS.custom} brands={brandStock} />
    </>
  )
}
