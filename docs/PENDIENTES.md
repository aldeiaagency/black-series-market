...
- [ ] **ARQ-1** — Patrón `data ?? []` en las 23 páginas públicas del catálogo (53 llamadas a Supabase, `app/(public)/**`) no distingue "sin resultados reales" de "la consulta falló": si Supabase devuelve error, la página renderiza como catálogo vacío en vez de mostrar un estado de error. Detectado 2026-09-07 al auditar `/dealers` (ahora `/showrooms`) — es sistémico, no un bug puntual de esa página, así que corregirlo ahí solo sería inconsistente con el resto del sitio. Pendiente decidir: ¿logging de errores (a dónde), estado de error visible al usuario, o se deja como fallo silencioso aceptado? Diferido — no bloqueante mientras el catálogo tenga pocos datos y la operación sea manual.
