...
- [x] E4 · FAQ/GEO: componente reutilizable `FaqSection` (server, `<details>` accesible + `FAQPage` JSON-LD, texto siempre en el DOM para rastreadores/LLMs). Añadido a **home** (8 Q&A: definición del servicio, "dónde comprar…", coste, confianza, contacto, cobertura nacional, alta profesional), **/coches** (5 Q&A: tipos, estado/historial, financiación, IVA, contacto) y **/motos** (5 Q&A: tipos, carnet, electrónica, financiación, cobertura) — solo en la vista sin filtros para no duplicar schema. Todo factual (basado en campos reales de la ficha), sin inventar precios/claims. Ya existían FAQPage en `precios` y `como-funciona` + schema extenso (8 olas: Car/Motorcycle, Offer, AutoDealer, ItemList, BreadcrumbList, glosario, guías Article, llms.txt). Canonical en home = el global del layout; contacto/precios/como-funciona ya tienen canonical propio.
